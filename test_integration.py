...
def test_preprocessor_integration():
    ...
    # 初始化 Preprocessor
    p = Preprocessor()
    print(f"✓ Preprocessor 初始化成功")
    print(f"  - DIM_OF_OBSERVATION: {p.dim_of_observation}")
    print(f"  - USE_NEW_REWARD_SYSTEM: {p.use_new_reward_system if hasattr(p, 'use_new_reward_system') else 'Unknown'}")
