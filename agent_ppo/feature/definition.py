...
def sample_process(collector) -> list:
    return collector.sample_process()


# Create the sample for the current frame
# 创建当前帧的样本
def build_frame(agent, observation) -> Frame:
...
class FrameCollector:
    ...
    def save_frame(self, rl_data_info, agent_id):
        # Keep the reward aligned to the transition that produced the next state.
        # The current frame is stored with reward=0, and the previous frame receives
        # the reward observed after this transition.
        reward = self._clip_reward(rl_data_info.reward)

        if len(self.rl_data_map[agent_id]) > 0:
            self._append_transition_reward(agent_id, reward, rl_data_info.value)

        rl_data_info.reward = 0
        self.rl_data_map[agent_id][rl_data_info.frame_no] = rl_data_info

    def _append_transition_reward(self, agent_id, reward, next_value):
        last_key = list(self.rl_data_map[agent_id].keys())[-1]
        last_rl_data_info = self.rl_data_map[agent_id][last_key]
        last_rl_data_info.next_value = next_value
    ...
    def _clip_reward(self, reward, max_val=100, min_val=-100):
        if reward > max_val:
            reward = max_val
        elif reward < min_val:
            reward = min_val
